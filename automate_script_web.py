...
def call_gemini_api(prompt, is_json=False):
    st.info(f"   ... 🤖 Sending request to AI (Model: {MODEL_NAME})...")
    try:
        prompt_full = (f"{prompt}\n\nPlease respond with only a valid JSON string (or a JSON list). "
                       f"Do not add any other text, explanations, or markdown.") if is_json else prompt
        response = model.generate_content(prompt_full)
        time.sleep(2)
        text_response = (getattr(response, "text", None) or "").strip()
        if is_json:
            text_response = re.sub(r"^```(?:json)?\s*|\s*```$", "", text_response, flags=re.IGNORECASE)
...
def parse_markdown_table(markdown_text):
    rows = []
    lines = [ln.strip() for ln in markdown_text.splitlines() if ln.strip().startswith("|")]
    if len(lines) < 3:
        return rows
...
def parse_script_sections(markdown_text):
    ...
    return {
        "title": _grab("Title") or "Title Not Found",
        "description": _grab("Description") or "Description Not Found",
        "hashtags": _grab("Hashtags") or "#hashtags #not #found",
        "thumbnail_prompts": _grab("Thumbnail Prompts") or "Thumbnails Not Found",
        "keywords": _grab("Keywords") or "Keywords Not Found",
    }
def main_automation(video_topic, video_duration_minutes):
    ...
    try:
        ...
        other_script_data = parse_script_sections(markdown_script)
...
{other_script_data.get('thumbnail_prompts', 'N/A')}
        ...
        {{
            "1. CORE_IDEA": {{
                ...
                "desired_duration_seconds": {__SCENE_DURATION_SECONDS__}
            }},
            ...
            "3. SETTING": {{
                ...
                "negative_prompt": "cartoon, animated, stylized, illustration, low-resolution, blurry, morphing artifacts, distorted features, unrealistic movement, fast motion, shaky cam, text, watermark, modern humans, modern clothing, anachronistic items, incorrect anatomy, unrealistic physics, gore, blood."
            }},
        ...
        """
json_template_string = json_template_string.replace("__SCENE_DURATION_SECONDS__", str(SCENE_DURATION_SECONDS))
